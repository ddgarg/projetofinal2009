...
if (_usetrigger!="NOTRIGGER") then {
	...
	if (isNil ("KRON_TRGFlag")) then {
		...
		if (_usetrigger!="SILENTTRIGGER") then {
			...
			call compile format["%1 setTriggerStatements['count thislist<=%6', 'titletext [''SETOR %2 ESTA LIMPO !'',''PLAIN''];''%2'' setmarkerpos %4;%3=true;', 'titletext [''SECTOR <%2> HAS BEEN RE-OCCUPIED'',''PLAIN''];''%2'' setmarkerpos %5;%3=false;']", _trgname,_areaname,_flgname,_markerhide,_markershow,_zoneempty];
		} else {
